...
class MessageButtonCacheManager {
    ...
    private struct CacheKeys {
        static let locationRecords = "MessageButtonCache_LocationRecords"
        static let loginRecords = "MessageButtonCache_LoginRecords"
        static let internalLoginRecords = "MessageButtonCache_InternalLoginRecords"
        static let userNameRecords = "MessageButtonCache_UserNameRecords"
        static let userAvatarRecords = "MessageButtonCache_UserAvatarRecords"
        static let lastUpdateTime = "MessageButtonCache_LastUpdateTime"
    }
    ...
    func cacheLocationRecords(_ records: [LocationRecord]) {
        do {
            ...
            UserDefaults.standard.set(data, forKey: CacheKeys.locationRecords)
            updateLastCacheTime()
        } catch {
        }
    }
    ...
    func cacheLoginRecords(_ records: [LoginRecord]) {
        do {
            let data = try JSONEncoder().encode(records)
            UserDefaults.standard.set(data, forKey: CacheKeys.loginRecords)
            updateLastCacheTime()
        } catch {
        }
    }
    ...
    func cacheInternalLoginRecords(_ records: [InternalLoginRecord]) {
        do {
            let data = try JSONEncoder().encode(records)
            UserDefaults.standard.set(data, forKey: CacheKeys.internalLoginRecords)
            updateLastCacheTime()
        } catch {
        }
    }
    ...
    func cacheUserNameRecords(_ records: [[String: Any]]) {
        do {
            ...
            UserDefaults.standard.set(data, forKey: CacheKeys.userNameRecords)
            updateLastCacheTime()
        } catch {
        }
    }
    ...
    func cacheUserAvatarRecords(_ records: [[String: Any]]) {
        do {
            let data = try JSONSerialization.data(withJSONObject: records)
            UserDefaults.standard.set(data, forKey: CacheKeys.userAvatarRecords)
            updateLastCacheTime()
        } catch {
        }
    }
    ...
    func getCachedLocationRecords() -> [LocationRecord]? {
        guard let data = UserDefaults.standard.data(forKey: CacheKeys.locationRecords),
              !isCacheExpired() else {
            return nil
        }
    ...
    func getCachedLoginRecords() -> [LoginRecord]? {
        guard let data = UserDefaults.standard.data(forKey: CacheKeys.loginRecords),
              !isCacheExpired() else {
            return nil
        }
    ...
    func getCachedInternalLoginRecords() -> [InternalLoginRecord]? {
        guard let data = UserDefaults.standard.data(forKey: CacheKeys.internalLoginRecords),
              !isCacheExpired() else {
            return nil
        }
    ...
    func getCachedUserNameRecords() -> [[String: Any]]? {
        guard let data = UserDefaults.standard.data(forKey: CacheKeys.userNameRecords),
              !isCacheExpired() else {
            return nil
        }
    ...
    func getCachedUserAvatarRecords() -> [[String: Any]]? {
        guard let data = UserDefaults.standard.data(forKey: CacheKeys.userAvatarRecords),
              !isCacheExpired() else {
            return nil
        }
    ...
    private func updateLastCacheTime() {
        UserDefaults.standard.set(Date(), forKey: CacheKeys.lastUpdateTime)
    }
    
    /// 检查缓存是否过期
    private func isCacheExpired() -> Bool {
        guard let lastUpdateTime = UserDefaults.standard.object(forKey: CacheKeys.lastUpdateTime) as? Date else {
            return true
        }
    ...
    func clearAllCaches() {
        ...
        UserDefaults.standard.removeObject(forKey: CacheKeys.userNameRecords)
        UserDefaults.standard.removeObject(forKey: CacheKeys.userAvatarRecords)
        UserDefaults.standard.removeObject(forKey: CacheKeys.lastUpdateTime)
    }
    ...
    func getCacheStats() -> [String: Any] {
        ...
        if let lastUpdateTime = UserDefaults.standard.object(forKey: CacheKeys.lastUpdateTime) as? Date {
            stats["lastUpdateTime"] = lastUpdateTime
            stats["isExpired"] = isCacheExpired()
        }
    ...
    func updateMessageCount(for friendId: String, count: Int) {
        let key = "MessageCount_\(friendId)"
