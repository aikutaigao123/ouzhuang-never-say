...
struct BearingUtils {
    ...
    // 🎯 新增：格式化方向显示，例如"179° 南"
    static func formatDirectionDisplay(_ bearing: Double) -> String {
        let angle = Int(bearing.rounded())
        let directionText = getDirectionText(bearing)
        // 简化方向文字：去掉"正"字，例如"正南" -> "南"
        let simplifiedDirection = directionText.replacingOccurrences(of: "正", with: "")
        return "\(angle)° \(simplifiedDirection)"
    }
}
