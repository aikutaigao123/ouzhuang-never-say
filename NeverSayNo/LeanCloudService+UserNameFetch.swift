                ...
                case .failure(let error):
                    ...
                    if error.localizedDescription.contains("404") || error.localizedDescription.contains("not found") {
                        self.createUserNameRecordTable { success in
                            if success {
                                // 表创建成功后，重新尝试获取用户名
                                self.fetchUserName(objectId: objectId, loginType: loginType, completion: completion)
                            } else {
                ...
                case .failure(let error):
                    ...
                    if error.localizedDescription.contains("404") || error.localizedDescription.contains("not found") {
                        self.createUserNameRecordTable { success in
                            if success {
                                // 表创建成功后，重新尝试获取邮箱
                                self.fetchUserEmail(objectId: objectId, loginType: loginType, completion: completion)
                            } else {
                                completion(nil, "表创建失败")
                            }
                        }
