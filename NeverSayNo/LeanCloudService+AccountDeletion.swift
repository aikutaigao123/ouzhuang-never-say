...
extension LeanCloudService {
    ...
    private func deleteRecordsFromTableWithPagination(
        ...
        // 查询当前批次的记录
        let _ = "🔍 [AccountDeletion] \(tableName): 开始查询 (field=\(fieldName), userId=\(userId), skip=\(skip), limit=\(limit))"
        ...
        _ = query.find { result in
            ...
            case .success(objects: let objects):
                ...
                // 打印查询到的记录信息（用于调试）
                let _ = "🔍 [AccountDeletion] \(tableName): 查询到 \(objects.count) 条记录 (skip=\(skip), limit=\(limit), userId=\(userId))"
                ...
                for (index, obj) in objects.prefix(3).enumerated() {
                    ...
                    let _ = "   [\(index+1)] objectId=\(objId), \(fieldName)=\(userIdValue)"
                ...
                if objects.count == 1 {
                    ...
                    let _ = "🗑️ [AccountDeletion] \(tableName): 尝试删除单个对象 objectId=\(objId)"

                    objects[0].delete { deleteResult in
                        switch deleteResult {
                        case .success:
                            let newTotalDeleted = totalDeleted + 1
                            let _ = "✅ [AccountDeletion] \(tableName): 成功删除对象 objectId=\(objId)"
                        ...
                        case .failure(let error):
                            ...
                            if error.code == 429 && retryCount < maxRetries {
                                ...
                                let _ = "⏳ [AccountDeletion] \(tableName): 删除遇到429错误，\(retryDelay)秒后重试 (第\(retryCount + 1)次/共\(maxRetries)次)"
                                ...
                                let _ = "❌ [AccountDeletion] \(tableName): 删除失败 objectId=\(objId), error=\(error.localizedDescription), code=\(error.code)"
                    ...
                    for i in stride(from: 0, to: objects.count, by: batchSize) {
                        ...
                        _ = LCObject.delete(batch) { deleteResult in
                            ...
                            case .failure(let error):
                                // 🎯 429错误：记录但不立即失败，等待重试
                                if error.code == 429 {
                                    let _ = "⏳ [AccountDeletion] \(tableName): 批量删除遇到429错误 (第 \(i/batchSize + 1) 批)，将在后续重试"
            ...
            case .failure(let error):
                ...
                if error.code == 429 && retryCount < maxRetries {
                    let retryDelay = Double(retryCount + 1) * 2.0 // 2秒、4秒、6秒
                    let _ = "⏳ [AccountDeletion] \(tableName): 查询遇到429错误，\(retryDelay)秒后重试 (第\(retryCount + 1)次/共\(maxRetries)次)"
