        ...
        NavigationStack {
            ...
            .alert("确认清除", isPresented: $showClearAlert) {
                ...
                Button("清除", role: .destructive) {
                    // 🚀 优化：立即清空本地数据，确保UI即时更新（必须先执行）
                    // 使用空数组赋值，确保SwiftUI检测到变化并立即刷新UI
                    currentHistory = []
            ...
            // 🚀 优化：监听 history.count 的变化，确保清除操作后UI正确更新
            .onChange(of: history.count) { oldCount, newCount in
                // 当 history 数量变化时，同步更新 currentHistory
                currentHistory = history
            }
        }
    }
