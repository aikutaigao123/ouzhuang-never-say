...
struct SearchUtils {
    ...
    static func showHistoricalMatch(
        ...
        for (_, _) in Thread.callStackSymbols.prefix(8).enumerated() {
        }
        
        // 🎯 新增：计算从点击寻找按钮到出现匹配对象的时间
        if UserDefaults.standard.object(forKey: "SearchButtonClickTime") as? Date != nil {
            // 清除 UserDefaults 中的时间记录，避免下次误用
            UserDefaults.standard.removeObject(forKey: "SearchButtonClickTime")
        }
