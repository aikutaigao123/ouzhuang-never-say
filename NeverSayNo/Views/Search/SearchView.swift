...
struct SearchView: View {
    ...
    var body: some View {
        VStack {
            ...
            SearchButton(
                ...
                onRandomMatch: { record in
                    // 🎯 新增：钻石为0时的随机匹配处理
                    searchViewModel.randomRecord = record
                    searchViewModel.isLoading = false
                    
                    // 🔧 修复：保存到历史记录（免费匹配也要保存）
                    self.addToHistory(record: record)
                }
            )
            .padding(.top, 20)
        ...
        .onChange(of: searchStateManager.isUserBlacklisted) { oldValue, newValue in
        }
        ...
        .sheet(isPresented: $showMessageSheet) {
            ...
            .onAppear {
                      ...
                      .onChange(of: showRankingSheet) { oldValue, newValue in
                      }
    ...
    private func addFavoriteRecord(userId: String, userName: String?, userEmail: String?, loginType: String?, userAvatar: String?, recordObjectId: String?) {
        ...
        if UserDefaultsManager.hasSentFriendRequestToUserInLastMinute(targetUserId: userId) {
        ...
        UserDefaultsManager.recordFriendRequestSentToUser(targetUserId: userId)
    ...
    // 🔧 新增：添加到历史记录的方法
    private func addToHistory(record: LocationRecord) {
        // 获取历史记录存储键
        let historyKey = getHistoryKey()
        
        // 从 UserDefaults 加载现有历史记录
        var history: [RandomMatchHistory] = []
        
        if let data = UserDefaults.standard.data(forKey: historyKey),
           let existingHistory = try? JSONDecoder().decode([RandomMatchHistory].self, from: data) {
            history = existingHistory
        }
        
        // 检查是否已存在该用户（避免重复）
        if let existingIndex = history.firstIndex(where: { $0.record.userId == record.userId }) {
            history.remove(at: existingIndex)
        }
        
        // 创建新的历史记录项
        let currentLocation = locationManager.location?.coordinate
        let newHistory = RandomMatchHistory(
            record: record,
            recordNumber: 1,  // 免费匹配统一使用序号1
        ...
        // 限制历史记录数量，最多保存217条
        if history.count > 217 {
            history = Array(history.prefix(217))
        }
        ...
        if let encoded = try? JSONEncoder().encode(history) {
            ...
            // 发送通知刷新历史列表
            NotificationCenter.default.post(name: NSNotification.Name("RefreshHistoryList"), object: nil)
        }
    }
    
    // 🔧 新增：获取历史记录存储键
    private func getHistoryKey() -> String {
        ...
        if let loginType = loginType {
            if loginType == "apple" {
                ...
                if !email.isEmpty {
                    return "randomMatchHistory_internal_\(email)"
                } else if let userId = userId {
