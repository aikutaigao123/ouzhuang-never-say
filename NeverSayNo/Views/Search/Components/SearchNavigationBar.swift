...
struct SearchNavigationBar: View {
    ...
    private var totalPatBadgeCount: Int {
        ...
        let count = patMessageUpdateManager.getTotalUnreadPatCount(forReceiverId: currentUserId)
        return count
    }
    ...
    private var messageBadgeCount: Int {
        ...
        let total = friendRequests + patBadge
        return total
    }
        ...
        HStack {
            ...
            Button(action: {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    let userNameText = Text(userManager.userNameFromServer ?? (userManager.currentUser?.fullName ?? "用户"))
                        .font(.headline)
                        .fontWeight(.semibold)
                        .lineLimit(1)
                        .truncationMode(.tail)
            ...
            Button(action: {
                ...
                ZStack(alignment: .topTrailing) { // 明确指定对齐方式
                    ...
                    // 🎯 新增：调试日志
                    let badgeCount = messageBadgeCount
                    if badgeCount > 0 {
                        Text("\(badgeCount)")
                            .font(.caption2)
                            .fontWeight(.bold)
