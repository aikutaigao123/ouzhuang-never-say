...
struct MatchResultCard: View {
    ...
    var body: some View {
        ...
        .onAppear {
            // 🎯 新增：计算从点击寻找按钮到匹配卡片出现的时间
            if let searchStartTime = UserDefaults.standard.object(forKey: "SearchButtonClickTime") as? Date {
                let _ = Date().timeIntervalSince(searchStartTime)
