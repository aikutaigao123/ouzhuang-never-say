...
struct SearchCompassView: View {
    ...
    var body: some View {
        ZStack {
            ...
            // 🎯 新增：点击手势 - 高手匹配卡片时切换背景和指针显示，默认背景时打开详情界面
            Color.clear
                .frame(width: 250, height: 250)
                .contentShape(Circle())
                .onTapGesture {
                    ...
                    if isTop3RankingUser {
                        // 🎯 修改：如果是前3名用户
                        if showAppIconBackground {
                            // 当前是App图标背景，切换为默认轮盘背景
                            withAnimation(.easeInOut(duration: 0.3)) {
                                showAppIconBackground = false
                            }
                        } else {
                            // 🎯 新增：当前是默认轮盘背景，打开全屏详情界面
                            showCompassDetailView = true
                        }
                    } else {
                        // 🎯 新增：如果不是前3名用户，点击时也打开全屏详情界面
                        showCompassDetailView = true
                    }
                }
        ...
        .onChange(of: showAppIconBackground) { oldValue, newValue in
        }
