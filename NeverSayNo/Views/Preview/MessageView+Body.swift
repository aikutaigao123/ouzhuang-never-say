    ...
    var body: some View {
        ...
        .onAppear {
            if let currentUser = userManager.currentUser {
                // 检查未读数量
                let _ = PatMessageUpdateManager.shared.getTotalUnreadPatCount(forReceiverId: currentUser.id)
            }
        ...
        NotificationCenter.default.addObserver(
            ...
            if !existingMessages.isEmpty || !existingPatMessages.isEmpty || !existingFriends.isEmpty {
                ...
                // 没有现有数据，先从本地加载拍一拍消息
                if let currentUser = userManager.currentUser {
                    var localPatMessages = UserDefaultsManager.getPatMessages(userId: currentUser.id)
                    
                    // 🎯 新增：根据 MarkAllAsReadTimestamp 标记已读状态
                    let markAllAsReadKey = "MarkAllAsReadTimestamp_\(currentUser.id)"
                    if let markAllAsReadTimestamp = UserDefaults.standard.object(forKey: markAllAsReadKey) as? Date {
                        for index in localPatMessages.indices {
                            let timeDifference = localPatMessages[index].timestamp.timeIntervalSince(markAllAsReadTimestamp)
                            if timeDifference <= 1.0 && !localPatMessages[index].isRead {
                                localPatMessages[index].isRead = true
                            }
                        }
                        // 保存更新后的已读状态
                        UserDefaultsManager.savePatMessages(localPatMessages, userId: currentUser.id)
                    }
