...
extension MessageView {
    ...
    internal func processLoadedMessages(_ messages: [MessageItem]) {
        ...
        if let markAllAsReadTime = markAllAsReadTimestamp {
            ...
            for index in mergedPatMessages.indices {
                let message = mergedPatMessages[index]
                // 如果消息的时间戳早于或等于 markAllAsReadTime（允许1秒容差），则标记为已读
                let timeDifference = message.timestamp.timeIntervalSince(markAllAsReadTime)
                if timeDifference <= 1.0 && !message.isRead {
                    mergedPatMessages[index].isRead = true
                    markedAsReadCount += 1
                }
            }
        ...
        updatePatMessagesUserNames(mergedPatMessages) { updatedMessages in
            DispatchQueue.main.async {
                ...
                // 更新未读消息计数
                let calculatedUnreadCount = self.calculateUnreadCount()
                self.unreadCount = calculatedUnreadCount
                
                // 🎯 新增：清除 PatMessageUpdateManager 的缓存，确保下次获取时是最新数据
                if let currentUser = self.userManager.currentUser {
                    PatMessageUpdateManager.shared.clearUnreadCountCache(for: currentUser.id)
                }
            }
        }
    }
    ...
    internal func markAllAsRead() {
        ...
        DispatchQueue.main.async {
            ...
            // 🎯 新增：清除 PatMessageUpdateManager 的缓存
            PatMessageUpdateManager.shared.clearUnreadCountCache(for: currentUser.id)
