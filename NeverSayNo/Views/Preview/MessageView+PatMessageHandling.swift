...
extension MessageView {
    ...
    private func sendPatMessageWithFriendName(currentUser: UserInfo, friendId: String, friendName: String) {
        ...
        // 🎯 新增：记录向特定用户发送拍一拍的时间（用于1分钟内3次限制）
        UserDefaultsManager.recordPatActionSentToUser(targetUserId: toUserId)
            ...
            DispatchQueue.main.async {
                if success {
                    ...
                    if !exists {
                        ...
                        // 🎯 修复：发送 PatMessageAdded 通知，让其他组件也能更新
                        NotificationCenter.default.post(
