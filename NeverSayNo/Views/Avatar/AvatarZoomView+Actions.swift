...
extension AvatarZoomView {
    ...
    private func proceedWithUnlockAfterSpend(
        ...
        // 🔍 详细追踪：记录每次累计消耗的更新
        if AvatarZoomView.totalDiamondsSpent != AvatarZoomView.totalUnlockCount * 5 {
        }
        
        // 🔍 详细追踪：记录每次累计消耗的更新
        
        // 📊 验证统计
        // 🔧 修复：使用 DiamondStore 的值而不是 DiamondManager 的值，因为 Combine 可能有延迟
        let finalDiamondsFromStore = diamondManager.diamondStore?.balance.amount ?? currentDiamonds
        let finalDiamondsFromManager = diamondManager.diamonds
        let finalDiamonds = finalDiamondsFromStore  // 使用 Store 的值，因为它是最新的
        
        // 🔍 调试：如果 Store 和 Manager 的值不一致，打印警告
        if finalDiamondsFromStore != finalDiamondsFromManager {
        }
        let finalAvatarCount = diamondManager.ownedAvatars.count
        // 🔧 修复：实际消耗应该基于会话开始值到当前值的差值，而不是累计值
        // 在连续解锁模式下，应该使用会话开始值（服务器值）来计算，因为本地值可能还没有同步
        let baseDiamondsForCalculation = AvatarZoomView.sessionStartServerDiamonds > 0 ? AvatarZoomView.sessionStartServerDiamonds : AvatarZoomView.sessionStartDiamonds
        let actualDiamondsSpent = baseDiamondsForCalculation - finalDiamonds
        let actualAvatarIncrease = finalAvatarCount - AvatarZoomView.sessionStartAvatarCount
        
        // 🔧 按照开发指南：计算基于服务器真实值的实际消耗（等待服务器同步完成）
        _ = AvatarZoomView.sessionStartServerDiamonds > 0 ? 
            AvatarZoomView.sessionStartServerDiamonds - finalDiamonds : actualDiamondsSpent
        
        // 🔍 计算期望的最终钻石数（基于服务器基准值）
        // 🔧 按照开发指南：如果已有服务器基准值，优先使用服务器值计算期望值
        let baseDiamonds = AvatarZoomView.sessionStartServerDiamonds > 0 ? AvatarZoomView.sessionStartServerDiamonds : AvatarZoomView.sessionStartDiamonds
        let expectedFinalDiamonds = baseDiamonds - AvatarZoomView.totalDiamondsSpent
        let finalDiamondsDifference = finalDiamonds - expectedFinalDiamonds
        ...
        // 🎯 验证解锁数与钻石消耗是否一致
        let expectedDiamondsSpent = AvatarZoomView.totalUnlockCount * 5
        
        // 🔧 新增：核心验证 - 消耗的钻石数必须等于解锁的头像数 × 5
        // 🔧 修复：在连续解锁模式下，使用累计消耗值来验证，因为本地钻石值可能因服务器刷新而被覆盖
        let expectedDiamondsFromUnlocks = actualAvatarIncrease * 5
        let actualDiamondsSpentFromUnlocks = AvatarZoomView.totalDiamondsSpent  // 使用累计消耗值
        
        // 核心验证：累计消耗值应该等于解锁头像数 × 5
        if actualDiamondsSpentFromUnlocks != expectedDiamondsFromUnlocks {
        }
        
        // 额外验证：基于钻石差值的验证（可能因刷新被覆盖而不准确）
        if actualDiamondsSpent != expectedDiamondsFromUnlocks {
        }
        
        if AvatarZoomView.totalDiamondsSpent != expectedDiamondsSpent {
        }
        
        if AvatarZoomView.totalDiamondsSpent != actualDiamondsSpent {
        }
        
        if AvatarZoomView.totalUnlockCount != actualAvatarIncrease {
        }
        
        if finalDiamondsDifference != 0 {
        }
