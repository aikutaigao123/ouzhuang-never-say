...
struct FriendRequestModal: View {
    ...
    @State private var canSendContactInquiry: Bool = true // 🎯 新增：是否可以发送询问联系方式是否真实
    @State private var showContactInquiryLimitAlert = false // 🎯 新增：显示询问限制提示
    @State private var contactInquiryLimitMessage = "" // 🎯 新增：询问限制提示信息
    @State private var showContactInquiryDuplicateAlert = false // 🎯 新增：显示重复发送询问提示
    ...
    // 🎯 新增：检查是否可以发送询问联系方式是否真实
    private func checkCanSendContactInquiry() {
        let (canSend, _) = UserDefaultsManager.canSendContactInquiry()
        canSendContactInquiry = canSend
    }
    ...
    var body: some View {
        ZStack {
            ...
            .alert("今日询问已超过上限", isPresented: $showContactInquiryLimitAlert) {
                Button("确定", role: .cancel) { }
            } message: {
                Text(contactInquiryLimitMessage)
            }
            .alert("请勿重复发送询问", isPresented: $showContactInquiryDuplicateAlert) {
                Button("确定", role: .cancel) { }
            } message: {
                Text("1分钟内已向该用户发送过询问，请稍后再试")
            }
        }
    }
    ...
    // 🎯 新增：发送询问联系方式是否真实（参考发送好友请求的实现）
    private func sendContactInquiry() {
        ...
        // 🎯 检查24小时内询问数量限制（在点击时检查，不依赖API结果）
        let (canSend, limitErrorMessage) = UserDefaultsManager.canSendContactInquiry()
        if !canSend {
            // 超过限制，显示弹窗提示
            contactInquiryLimitMessage = limitErrorMessage
            showContactInquiryLimitAlert = true
            return
        }
        
        // 🎯 检查是否在1分钟内向同一用户发送过询问
        if UserDefaultsManager.hasSentContactInquiryToUserInLastMinute(targetUserId: record.userId) {
            showContactInquiryDuplicateAlert = true
            return
        }
        
        // 🎯 立即记录发送时间（在点击时记录，不依赖API结果）
        UserDefaultsManager.recordContactInquirySent(to: record.userId)
        UserDefaultsManager.recordContactInquirySentToUser(targetUserId: record.userId)
        ...
        // 🎯 新增：先创建数据库记录（类似好友申请）
        ContactInquiryManager.shared.sendContactInquiry(to: userId) { success, errorMessage in
            if success {
                // 数据库记录创建成功后，发送 IM 消息作为通知（支持离线推送）
                PatConversationManager.shared.sendContactInquiryMessage(
                    fromUserId: currentUserId,
                    toUserId: userId,
                    fromUserName: senderName.isEmpty ? currentUserId : senderName,
                    toUserName: userNameValue
                ) { imSuccess, imErrorMessage in
                    if imSuccess {
                        // 🎯 发送成功后，更新 LoginRecord 表（参考好友请求的实现）
                        self.updateLoginRecordAfterContactInquiry(senderId: currentUserId, senderName: senderName)
                    }
                }
            }
        }
    ...
    // 🎯 新增：发送询问联系方式是否真实后更新 LoginRecord 表
    private func updateLoginRecordAfterContactInquiry(senderId: String, senderName: String) {
