...
class PatConversationManager: NSObject {
     ...
     * 🎯 新增：发送联系方式真实回复消息
     * - Parameters:
     *   - fromUserId: 发送者用户ID（当前用户）
     *   - toUserId: 接收者用户ID（询问者）
     *   - fromUserName: 发送者用户名
     *   - toUserName: 接收者用户名
     *   - completion: 完成回调
     */
    func sendContactInquiryReply(fromUserId: String, toUserId: String, fromUserName: String, toUserName: String, completion: @escaping (Bool, String?) -> Void) {
        // 生成对话ID（使用与拍一拍相同的对话）
        let conversationId = generatePatConversationId(fromUserId: fromUserId, toUserId: toUserId)
        ...
        if !isConnected {
            ...
            imClientManager.loginIMClient(userId: currentUserId) { [weak self] success, error in
                if success {
                    // 连接成功后继续发送消息
                    self?.continueSendContactInquiryReply(
                        conversationId: conversationId,
                        fromUserId: fromUserId,
                        toUserId: toUserId,
                        fromUserName: safeFromUserName,
                    ...
                    // 即使IM连接失败，回复消息也已经成功，只是无法发送即时通知
                    completion(false, "IM连接失败，但回复消息已成功: \(error ?? "未知错误")")
                }
            }
            return
        }
        ...
        // IM已连接，继续发送消息
        continueSendContactInquiryReply(
            conversationId: conversationId,
            fromUserId: fromUserId,
            toUserId: toUserId,
            fromUserName: safeFromUserName,
     ...
     * 继续发送联系方式真实回复消息（辅助方法）
     */
    private func continueSendContactInquiryReply(
        conversationId: String,
        fromUserId: String,
        toUserId: String,
        fromUserName: String,
        ...
        // 检查是否已有缓存的对话
        if let cachedConversation = conversations[conversationId] {
            sendContactInquiryReplyToConversation(cachedConversation, fromUserId: fromUserId, toUserId: toUserId, fromUserName: fromUserName, toUserName: toUserName, content: content, completion: completion)
            return
        }
        ...
        createOrGetPatConversation(fromUserId: fromUserId, toUserId: toUserId) { [weak self] conversation, error in
            ...
            // 发送消息
            self?.sendContactInquiryReplyToConversation(conversation, fromUserId: fromUserId, toUserId: toUserId, fromUserName: fromUserName, toUserName: toUserName, content: content, completion: completion)
        }
    }
    
    /**
     * 向对话发送联系方式真实回复消息（带推送配置）
     */
    private func sendContactInquiryReplyToConversation(_ conversation: IMConversation, fromUserId: String, toUserId: String, fromUserName: String, toUserName: String, content: String, completion: @escaping (Bool, String?) -> Void) {
        // 创建文本消息
        let message = IMTextMessage(text: content)

        // 🔧 根据离线消息推送文档：客户端发送消息时指定推送信息
        let pushData: [String: Any] = [
            "alert": "\(fromUserName) 回复：我的联系方式是真实的",
            "badge": "Increment",
            "sound": "default",
            "content-available": 1,  // 🔧 关键：必须包含此字段，后台才能处理推送
            "messageType": "contact_inquiry_reply",  // 自定义字段：消息类型
            "senderName": fromUserName,
            "receiverName": toUserName,
            "senderId": fromUserId,
            "receiverId": toUserId
    ...
    /**
     * 发送询问联系方式是否真实消息（类似好友申请消息，支持离线推送）
     * - Parameters:
     *   - fromUserId: 发送者用户ID
     *   - toUserId: 接收者用户ID
     *   - fromUserName: 发送者用户名
     ...
     *   - completion: 完成回调
     */
    func sendContactInquiryMessage(fromUserId: String, toUserId: String, fromUserName: String, toUserName: String, completion: @escaping (Bool, String?) -> Void) {
        // 生成对话ID（使用与拍一拍相同的对话）
        let conversationId = generatePatConversationId(fromUserId: fromUserId, toUserId: toUserId)
        ...
        // 消息内容
        let content = "\(safeFromUserName) 询问你的联系方式是否真实"
        ...
        if !isConnected {
            ...
            imClientManager.loginIMClient(userId: currentUserId) { [weak self] success, error in
                if success {
                    // 连接成功后继续发送消息
                    self?.continueSendContactInquiryMessage(
                        conversationId: conversationId,
                        fromUserId: fromUserId,
                        toUserId: toUserId,
                        fromUserName: safeFromUserName,
                    ...
                    // 即使IM连接失败，询问消息也已经成功，只是无法发送即时通知
                    completion(false, "IM连接失败，但询问消息已成功: \(error ?? "未知错误")")
                }
            }
            return
        }
        ...
        // IM已连接，继续发送消息
        continueSendContactInquiryMessage(
            conversationId: conversationId,
            fromUserId: fromUserId,
            toUserId: toUserId,
            fromUserName: safeFromUserName,
    ...
    /**
     * 继续发送询问联系方式是否真实消息（辅助方法）
     */
    private func continueSendContactInquiryMessage(
        conversationId: String,
        fromUserId: String,
        toUserId: String,
        fromUserName: String,
        ...
        // 检查是否已有缓存的对话
        if let cachedConversation = conversations[conversationId] {
            sendContactInquiryMessageToConversation(cachedConversation, fromUserId: fromUserId, toUserId: toUserId, fromUserName: fromUserName, toUserName: toUserName, content: content, completion: completion)
            return
        }
        ...
        createOrGetPatConversation(fromUserId: fromUserId, toUserId: toUserId) { [weak self] conversation, error in
            ...
            // 发送消息
            self?.sendContactInquiryMessageToConversation(conversation, fromUserId: fromUserId, toUserId: toUserId, fromUserName: fromUserName, toUserName: toUserName, content: content, completion: completion)
        }
    }
    
    /**
     * 向对话发送询问联系方式是否真实消息（带推送配置）
     */
    private func sendContactInquiryMessageToConversation(_ conversation: IMConversation, fromUserId: String, toUserId: String, fromUserName: String, toUserName: String, content: String, completion: @escaping (Bool, String?) -> Void) {
        // 创建文本消息
        let message = IMTextMessage(text: content)
        ...
        // 关键：必须包含 content-available: 1，否则后台无法触发 didReceiveRemoteNotification
        let pushData: [String: Any] = [
            "alert": "\(fromUserName) 询问你的联系方式是否真实",
            "badge": "Increment",
            "sound": "default",
            "content-available": 1,  // 🔧 关键：必须包含此字段，后台才能处理推送
            "messageType": "contact_inquiry",  // 自定义字段：消息类型
            "senderName": fromUserName,
            "receiverName": toUserName,
            "senderId": fromUserId,
            "receiverId": toUserId
    ...
    private func handleReceivedMessage(_ message: IMMessage) {
        ...
        // 🎯 新增：检查是否是联系方式真实回复消息
        if content.contains("我的联系方式是真实的") {
            // 检查是否是发给当前用户的消息
            ...
            if isMessageForCurrentUser {
                // 从消息内容中提取发送者用户名
                // 消息格式："{fromUserName} 回复：我的联系方式是真实的" 或 "我的联系方式是真实的"
                // 由于消息内容就是"我的联系方式是真实的"，我们需要从推送数据或其他地方获取发送者信息
                // 这里我们使用 fromUserId 查询用户名
                LeanCloudService.shared.fetchUserNameByUserId(objectId: fromUserId) { senderName, _ in
                    DispatchQueue.main.async {
                        let finalSenderName = senderName ?? "未知用户"
        ...
        // 🎯 新增：检查是否是询问联系方式是否真实消息
        if content.contains("询问你的联系方式是否真实") {
            // 检查是否是发给当前用户的消息
            var isMessageForCurrentUser = false
            ...
            if isMessageForCurrentUser {
                // 从消息内容中提取发送者用户名
                // 消息格式："{fromUserName} 询问你的联系方式是否真实"
                let senderName = extractSenderNameFromContactInquiry(content: content) ?? "未知用户"
        ...
        // 🔧 修复：处理好友申请消息
            ...
            if isMessageForCurrentUser {
                ...
                // 🎯 方案2：收到 friend_request 消息时，同步增加 NewFriendsCountManager 的 count
    ...
    /// 从消息内容中提取接收者用户名
    /**
     * 从询问联系方式是否真实消息中提取发送者用户名
     * 消息格式："{fromUserName} 询问你的联系方式是否真实"
     */
    private func extractSenderNameFromContactInquiry(content: String) -> String? {
        if let range = content.range(of: " 询问你的联系方式是否真实") {
            let senderName = String(content[..<range.lowerBound])
            return senderName.isEmpty ? nil : senderName
        }
        return nil
    }
