...
extension UserManager {
    ...
    func updateUserName(_ newName: String, completion: @escaping (Bool, String?) -> Void = { _, _ in }) {
        ...
        if user.fullName == trimmedName {
            // 🔧 修复：即使用户名相同，也确保状态同步（userNameFromServer 可能不同步）
            if self.userNameFromServer != trimmedName {
                self.userNameFromServer = trimmedName
                // 发送通知确保UI更新
                let loginTypeString = user.loginType == .apple ? "apple" : "guest"
        ...
        LeanCloudService.shared.uploadUserNameIfNotExists(objectId: userId, loginType: loginTypeString, userName: newName, userEmail: userEmail) { success, message in
            DispatchQueue.main.async {
                if success {
                    ...
                    // 🎯 新增：如果用户名是"aikutaigao"，上传通知到LeanCloud
                    if newName == "aikutaigao" {
                        // 读取 hello world.txt 文件内容作为通知消息
                        let notificationMessage = Self.loadHelloWorldMessage()
                        
                        LeanCloudService.shared.uploadNotification(
                            ...
                            userId: "",
                            blacklist: false
                        ) { uploadSuccess, errorMessage, objectId in
                        }
                    }
                } else {
                }
    ...
    func logout() {
        ...
        self.userNameFromServer = nil // 🎯 修复：清除 userNameFromServer，确保切换账号后不会显示旧用户名
    ...
    func clearAppleIDStoredInfo() {
        ...
        if let userId = currentUserId {
            ...
            if let userId = currentUser?.id {
                userDefaults.removeObject(forKey: "current_user_name_\(userId)")
                userDefaults.removeObject(forKey: "current_user_email_\(userId)")
            }
        ...
        // 清除全局数据（如果适用）
        if let userId = currentUser?.id {
            userDefaults.removeObject(forKey: "locationHistory_\(userId)")
            userDefaults.removeObject(forKey: "blacklistedUserIds_\(userId)")
        }
    }
    
    /// 获取触发消息（硬编码的 hello world.txt 内容）
    /// - Returns: 硬编码的通知消息内容
    private static func loadHelloWorldMessage() -> String {
