...
extension LeanCloudService {
    ...
    private func parseLocationRecord(from dict: [String: Any]) -> LocationRecord? {
        ...
        if let deviceTime = dict["deviceTime"] as? String, !deviceTime.isEmpty {
            timestamp = deviceTime
        } else if let createdAtDict = dict["createdAt"] as? [String: Any],
                  let isoString = createdAtDict["iso"] as? String {
            // LeanCloud 的 createdAt 格式：{"__type": "Date", "iso": "2025-12-21T00:16:30.000Z"}
            timestamp = isoString
        } else if let createdAtString = dict["createdAt"] as? String, !createdAtString.isEmpty {
