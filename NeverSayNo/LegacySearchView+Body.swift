...
extension LegacySearchView {
    ...
    /// 打开 App Store（用于触发消息的通知）
    private func openAppStore() {
        // 🎯 使用真实的 App Store ID
        let appStoreId = "6755204835"
        
        // 方法1: 使用 itms-apps:// 协议（优先，直接打开 App Store 应用）
            ...
            if UIApplication.shared.canOpenURL(url) {
                UIApplication.shared.open(url) { success in
                    if success {
                        ...
                        // 尝试备用方法
                        self.openAppStoreFallback(appStoreId: appStoreId)
                    }
                }
                return
            }
        ...
        // 方法2: 使用 https://apps.apple.com（备用，在浏览器中打开）
        openAppStoreFallback(appStoreId: appStoreId)
    }
    
    /// 打开 App Store 的备用方法
    private func openAppStoreFallback(appStoreId: String) {
        // 使用 https://apps.apple.com 打开（在浏览器或 Safari 中打开）
