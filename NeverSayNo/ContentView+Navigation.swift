        ...
        if loginType != .guest {
            ...
            LeanCloudService.shared.fetchUserEmailByUserId(objectId: userId) { email, error in
                DispatchQueue.main.async {
                    ...
                    self.continueMessageButtonTapForTabBar(userId: userId)
                }
            }
        } else {
            // 游客账号，直接继续执行
            continueMessageButtonTapForTabBar(userId: userId)
        }
    }
    
    /// 继续执行消息按钮点击（从底部TabBar，显示消息界面）
    private func continueMessageButtonTapForTabBar(userId: String) {
        // 记录点击
