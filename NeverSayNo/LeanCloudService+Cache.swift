...
extension LeanCloudService {
    ...
    private func cleanupExpiredCache() {
        ...
        // 🎯 清理黑名单缓存（按用户隔离）
        blacklistCache = blacklistCache.filter { _, value in
            currentTime.timeIntervalSince(value.timestamp) <= blacklistCacheExpirationInterval
        }
    ...
    func getCachedUserName(for userId: String) -> String? {
        ...
        guard let cacheEntry = userNameCache[userId] else {
            return nil
        }
    ...
    func clearCacheForUser(_ userId: String) {
        ...
        userDiamondsCache.removeValue(forKey: userId)
        onlineStatusCache.removeValue(forKey: userId)
        blacklistCache.removeValue(forKey: userId) // 🎯 清除指定用户的黑名单缓存
    }
    ...
    func clearAllCache() {
        ...
        blacklistCache.removeAll() // 🎯 清除所有用户的黑名单缓存
    }
    ...
    func clearCurrentUserCache() {
        ...
        guard let currentUserId = UserDefaultsManager.getCurrentUserId() else { return }
        clearCacheForUser(currentUserId)
    }
    
    // 🎯 获取缓存的黑名单（按用户隔离）
    func getCachedBlacklist(userId: String? = nil) -> [String]? {
        cacheLock.lock()
        defer { cacheLock.unlock() }
        
        // 🎯 获取当前用户ID
        let currentUserId = userId ?? UserDefaultsManager.getCurrentUserId() ?? ""
        guard !currentUserId.isEmpty else {
            return nil
        }
        
        guard let cacheEntry = blacklistCache[currentUserId] else {
            return nil
        }
        ...
        if timeSinceCache > blacklistCacheExpirationInterval {
            blacklistCache.removeValue(forKey: currentUserId)
            return nil
        }
    ...
    // 🎯 缓存黑名单（按用户隔离）
    func cacheBlacklist(_ blacklist: [String], userId: String? = nil) {
        cacheLock.lock()
        defer { cacheLock.unlock() }
        ...
        blacklistCache[currentUserId] = (blacklist: blacklist, timestamp: Date())
    }
